...
interface BossKillsPieComponentProps {
    players: Player[];
}
...
function getBossColor(bossName: string): string {
    // Generate a consistent color based on the boss name
    let hash = 0;
...
export function BossKillsPieComponent({ players }: BossKillsPieComponentProps) {
    const [chartType, setChartType] = useState<'pie' | 'bar'>('pie');
    const [hoveredSlice, setHoveredSlice] = useState<string | null>(null);
    
    // Calculate total boss kills across all players
        ...
        players.forEach(player => {
            ...
            Object.entries(activities).forEach(([activityName, activity]) => {
                if (isBossActivity(activityName) && activity.score > 0) {
                    ...
                    kills[bossName] = (kills[bossName] || 0) + activity.score;
                }
            });
        });
