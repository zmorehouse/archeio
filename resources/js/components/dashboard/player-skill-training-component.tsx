...
interface PlayerSkillTrainingComponentProps {
    playerId: number;
...
export function PlayerSkillTrainingComponent({ playerId: _playerId, historicalStats = [] }: PlayerSkillTrainingComponentProps) {
    ...
    const calculateSkillXpGained = useCallback(() => {
        ...
        if (historicalStats.length < 2) return skillXpGained;
        ...
        const recentStats = historicalStats.filter(stat => new Date(stat.fetched_at) >= cutoffDate);
        if (recentStats.length < 2) return skillXpGained;
        ...
        SKILL_ORDER.forEach(skillName => {
            ...
            if (xpGained > 0) {
                skillXpGained[skillName] = xpGained;
            }
        });

        return skillXpGained;
    }, [historicalStats, period]);
